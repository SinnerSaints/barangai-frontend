    ...
    <section className={`relative mt-6 rounded-[2rem] border p-6 shadow-xl lg:p-8 ${isDark ? "border-white/10 bg-zinc-950/85 text-white" : "border-gray-200 bg-white/90 text-[#034440]"}`}>
            ...
            <div className="mt-5 space-y-3 text-sm">
              {RATINGS.map((rating) => (
                <div key={rating.value} className="flex items-center justify-between rounded-2xl bg-white/10 px-4 py-3">
                  ...
                  <span className="text-lg font-bold text-accentGreen">{rating.value}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {viewState === "ready" && attempt && (
        <div className="mt-8">
          ...
          <div className={`mb-6 rounded-[1.25rem] p-4 ${isDark ? "bg-white/5" : "bg-[#f5faf7]"}`}>
            <p className="mb-3 text-xs uppercase tracking-[0.2em] opacity-70">Rating Guide</p>
            <div className="flex flex-wrap gap-2 sm:gap-4">
              {RATINGS.map((rating) => (
                <div key={rating.value} className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium ${isDark ? "bg-black/20 text-zinc-300" : "bg-white text-gray-700 shadow-sm"}`}>
                  <span className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold ${isDark ? "bg-white/10 text-accentGreen" : "bg-brandGreen/10 text-brandGreen"}`}>{rating.value}</span>
                  {rating.label}
                </div>
              ))}
            </div>
          ...
          {/* SCROLLING QUESTIONS CONTAINER (No Dock, No Scrollbars) */}
          <div className={`rounded-[2rem] border ${isDark ? "border-white/10 bg-zinc-950" : "border-gray-200 bg-white"}`}>
            <div 
              className="max-h-[60vh] overflow-y-auto p-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
            >
              <div className="space-y-12">
                {Object.entries(groupedQuestions).map(([category, questions]) => (
                  <div key={category}>
                    <div className="mb-5 flex items-center gap-3">
                      <h3 className="text-xl font-bold">{CATEGORY_LABELS[category] || category}</h3>
                      <div className={`h-px flex-1 ${isDark ? "bg-white/10" : "bg-gray-200"}`} />
                    </div>

                    <div className="space-y-4">
                      {questions.map((question) => (
                        <div key={question.id} className={`rounded-2xl border p-5 ${isDark ? "border-white/10 bg-white/5" : "border-gray-100 bg-[#fbfdfb]"}`}>
                          <p className="text-base font-medium leading-7">{question.order}. {question.question_text}</p>
                            ...
                            {RATINGS.map((rating) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex flex-1 flex-col items-center justify-center min-w-[80px] rounded-2xl px-2 py-3 transition-all duration-200 ${
                                    active
                                      ? isDark
                                        ? "bg-accentGreen text-black scale-[1.02] shadow-md"
                                        : "bg-brandGreen text-white scale-[1.02] shadow-md"
                                      : isDark
                                        ? "bg-black/20 text-zinc-400 hover:bg-white/10 hover:text-zinc-200"
                                        : "bg-white border border-gray-200 text-gray-600 hover:border-brandGreen/40 hover:bg-brandGreen/5"
                                  }`}
                                >
                                  <span className="text-lg font-bold">{rating.value}</span>
                                  <span className="mt-1 text-[10px] uppercase tracking-wider opacity-90 text-center">
                                    {rating.label}
                                  </span>
                                </button>
                              );
