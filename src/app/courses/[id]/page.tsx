    ...
    const fetchLesson = async () => {
      ...
      try {
        ...
        const mappedLessons = Array.isArray(listData)
          ...
          : [];
        const matchedLesson = mappedLessons.find((item) => Number(item.id) === courseId);
      ...
      <main className={`flex-1 p-8 lg:p-12 relative overflow-hidden ${isDark ? "text-white" : "text-black"}`}>
        ...
        <div className="relative z-10 flex h-full min-h-screen flex-col">
          ...
          >
            ...
            <div className="mt-6 mb-6 flex items-center justify-between gap-4">
              <Link
                ...
                className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold ${
                  isDark ? "bg-white/10 text-white" : "bg-white/80 text-[#034440]"
                }`}
              >
                <ArrowLeft size={16} />
                Back to Courses
            ...
            {loading ? (
              <div className="p-10 text-center font-bold text-brandGreen">Loading course...</div>
            ) : error ? (
              <div className={`rounded-3xl border p-8 ${isDark ? "border-white/10 bg-zinc-900/80" : "border-gray-200 bg-white/90"}`}>
                <h1 className="text-2xl font-bold">Course unavailable</h1>
                <p className={`mt-3 ${isDark ? "text-zinc-300" : "text-gray-600"}`}>{error}</p>
              </div>
            ) : lesson ? (
              <section className={`flex min-h-0 flex-1 flex-col overflow-hidden rounded-[2rem] border shadow-xl ${isDark ? "border-white/10 bg-zinc-950/90" : "border-gray-200 bg-white/90"}`}>
                <div className={`border-b px-8 py-7 ${isDark ? "border-white/10" : "border-gray-200"}`}>
                  <div className="mb-4 flex flex-wrap items-center gap-3">
                    <span className={`rounded-full px-3 py-1 text-sm font-semibold ${isDark ? "bg-zinc-800 text-zinc-200" : "bg-brandGreen/10 text-brandGreen"}`}>
                      {lesson.topic}
                    </span>
                    <span className={`inline-flex items-center gap-2 text-sm ${isDark ? "text-zinc-400" : "text-gray-500"}`}>
                      <Calendar size={14} />
                      {createdAt}
                    </span>
                  </div>

                  <h1 className="max-w-4xl text-3xl font-bold leading-tight lg:text-4xl">{lesson.title}</h1>

                  <div className={`mt-6 flex flex-wrap gap-6 text-sm ${isDark ? "text-zinc-300" : "text-gray-600"}`}>
                ...
                <div className="min-h-0 flex-1 overflow-y-auto px-8 py-8">
                  <div className="max-w-4xl">
                    <h2 className={`mb-4 text-lg font-semibold ${isDark ? "text-zinc-100" : "text-[#034440]"}`}>Course Content</h2>
                    <div className={`whitespace-pre-wrap text-base leading-8 ${isDark ? "text-zinc-200" : "text-gray-700"}`}>
